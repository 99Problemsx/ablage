...
class Battle::AI
  ...
  def choose_item_to_use
    ...
    # Script-defined trainer data can contain a stale/empty item entry while
    # saves or PBS data are being migrated. Never pass those entries into the
    # battle item handlers.
    items = items.compact.select { |item| GameData::Item.get(item) rescue false }
