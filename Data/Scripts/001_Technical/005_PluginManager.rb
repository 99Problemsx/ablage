...
module PluginManager
  # One line per plugin is useful while resolving load order, but makes every
  # normal debug boot dozens of lines long. Dependency failures still abort
  # with their full plugin/script context.
  VERBOSE_LOAD_OUTPUT = false
  ...
  def self.runPlugins
    ...
    scripts.each do |plugin|
      ...
      script.each do |scr|
        ...
        begin
          ...
          if VERBOSE_LOAD_OUTPUT && !echoed_plugins.include?(name)
            Console.echoln_li("Loaded plugin: ==#{name}== (ver. #{meta[:version]})")
          end
    ...
    if !incompatible_plugins.empty?
      Console.echo_warn("#{incompatible_plugins.length} plugin(s) declare another Essentials version; loaded for the v#{Essentials::VERSION} port anyway.")
    end
    if scripts.length > 0
