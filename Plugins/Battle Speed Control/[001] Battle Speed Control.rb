...
module BattleSpeedControl
  ...
  # Trigger key for toggling speed during battle (Input::ACTION, Input::L, etc.)
  # Set to nil to disable trigger key
  # v22-Port: Input::SPECIAL existiert nicht mehr -> Input::ACTION ist die Z/X-Taste
  TRIGGER_KEY = Input::ACTION  # Z key on keyboard, R button on controller
...
if BattleSpeedControl::SHOW_IN_OPTIONS
  MenuHandlers.add(:options_menu, :battle_speed, {
    "page"        => :gameplay,   # v22-Port
    "name"        => _INTL("Battle Speed"),
    "order"       => 25,
    "type"        => :array,      # v22-Port: EnumOption -> :array
...
# Debug Info
#===============================================================================
speed_message = "v1.1.0 stages=#{BattleSpeedControl::SPEED_STAGES.join(',')} " \
                "default=x#{BattleSpeedControl::SPEED_STAGES[BattleSpeedControl::DEFAULT_SPEED]} " \
                "hotkey=#{BattleSpeedControl::TRIGGER_KEY ? 'on' : 'off'}"
if defined?(SCScripts) && SCScripts.respond_to?(:log)
  SCScripts.log(speed_message, :info, :battle)
else
  puts "[Battle Speed] #{speed_message}"
end
