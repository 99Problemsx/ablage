...
EventHandlers.add(:on_pokemon_received, :monotype_check,
  proc { |pokemon|
    # v22-Port: on_pokemon_received uebergibt das Pokemon direkt (nicht |sender, [pkmn]|).
    next if !pokemon
    next if !ChallengeModes.on?(:MONOTYPE_MODE)
    next if ChallengeModes.valid_monotype_catch?(pokemon)
    ...
    # v22-Port: Empfang kann nicht mehr per e[0]=nil abgebrochen werden;
    # Monotype-Enforcement laeuft ueber pbCanSwitchIn?.
  }
)
