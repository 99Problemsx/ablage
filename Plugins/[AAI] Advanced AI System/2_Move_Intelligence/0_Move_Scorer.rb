...
class Battle::AI
  ...
  def score_move_advanced(move, user, target, skill)
    ...
    factors = AdvancedAI::DEBUG_MODE ? {} : nil
  ...
  def check_priority_tier_moves(move, user, target, skill)
    ...
    # GUARD: winning the turn beats setting up. If we can already secure a KO on
    # a foe this turn, don't let a hazard / status / screen / setup move's
    # priority boost — or worse, its auto-select (which bypasses even the -120
    # "KO available" malus in score_move_advanced) — hijack the pick. Fall through
    # to normal scoring so the damaging KO move wins. This is what stopped the
    # "spam Stealth Rock / Spore / Yawn and self-destruct instead of just killing
    # it" behaviour. Recovery is exempt: healing can still be correct with a KO
    # available, and it has its own OHKO-risk handling below.
    is_recovery = move.function_code.start_with?("HealUser")
    if skill >= 70 && !is_recovery && can_ko_opponent?(user)
      AdvancedAI.log("#{move.name}: tactical priority suppressed — can KO this turn", "Priority")
