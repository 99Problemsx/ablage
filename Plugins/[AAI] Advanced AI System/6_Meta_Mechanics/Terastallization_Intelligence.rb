...
#
# DBK_006 already defines the correct Battle::Battler#tera? / #tera_type
# (delegating to @pokemon), and DBK_000 supplies false-returning placeholders
# when Terastallization isn't installed. AAI loads AFTER DBK, so defining these
# unconditionally CLOBBERED DBK's working versions with ones gated on
# Settings::TERASTALLIZE_TRIGGER_KEY — a constant that exists nowhere in this
# project — making battler.tera? always false and battler.tera_type always nil
# even while a Pokémon is Terastallized. Define each only as a fallback (when
# nothing else provides it) and drop the phantom-constant gate entirely.
class Battle::Battler
  unless method_defined?(:can_terastallize?)
    def can_terastallize?
      return false unless @battle.respond_to?(:pbCanTerastallize?)
      return @battle.pbCanTerastallize?(@index)
    end
  end

  unless method_defined?(:tera?)
    def tera?
      return @pokemon&.tera? || false
    end
  end

  unless method_defined?(:tera_type)
    def tera_type
