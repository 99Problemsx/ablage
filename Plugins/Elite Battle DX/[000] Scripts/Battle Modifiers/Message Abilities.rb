...
class Battle::Scene
  ...
  def pbShowAbilitySplash(battler = nil, ability = nil)
    ...
    pbDrawEBDXOutlineText(bitmap, 28, 4, bitmap.width - 38, bitmap.font.size, _INTL("{1}'s", battler.name), baseColor, Color.new(0, 0, 0, 125), 0)
    pbDrawEBDXOutlineText(bitmap, 0, bitmap.height/2 + 4, bitmap.width - 28, bitmap.font.size, "#{effect}", baseColor, Color.new(0, 0, 0, 125), 2)
