...
class Battle::Scene
  ...
  def sendoutShinyPkmn(sendOuts)
    sendOuts.each do |b|
      ...
      # Essentials v22 removed Battle#showAnims. Animation suppression is now
      # represented by the :no_battle_animations battle rule.
      next if @battle.rules[:no_battle_animations] ||
              !shinyBattler?(@battlers[b[0]])
