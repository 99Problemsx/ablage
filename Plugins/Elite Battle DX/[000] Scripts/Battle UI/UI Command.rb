...
class Battle::Scene
  ...
  def pbCommandMenuEx(idxBattler, texts, mode = 0)
    ...
    # unselect databoxes
    self.pbDeselectAll
    # EBDX v21 returned numeric command IDs. Essentials v22 dispatches command
    # symbols instead (:fight, :bag, :pokemon, :run, ...). Convert only at the
    # scene boundary so EBDX's internal animations can keep using their IDs.
    return ebdxCommandForV22(@ret, texts, idxBattler)
  end

  #-----------------------------------------------------------------------------
  # translate legacy EBDX command IDs to Essentials v22 command symbols
  #-----------------------------------------------------------------------------
  def ebdxCommandForV22(command, available_commands, idxBattler)
    commands = Array(available_commands)
    return :cancel if command == -1
    return :debug if command == -2 || command == 4

    if @safaribattle
      safari_commands = [:throw_ball, :throw_bait, :throw_rock, :run]
      return safari_commands[command] || :cancel
    end

    case command
    when 0
      return :fight2 if commands.include?(:fight2)
      return :fight
    when 1
      return :bag if commands.include?(:bag)
      return :throw_ball_contest if commands.include?(:throw_ball_contest)
      return :throw_ball if commands.include?(:throw_ball)
    when 2
      return :pokemon
    when 3
      battler = @battle.battlers[idxBattler]
      if commands.include?(:call) && battler&.shadowPokemon? && battler.inHyperMode?
        return :call
      end
      return :run if commands.include?(:run)
      return :cancel if commands.include?(:cancel)
      return :call if commands.include?(:call)
    end
    return :cancel
  end
  #-----------------------------------------------------------------------------
  ...
  def refreshCommands(index)
    ...
    for i in 0...cmds.length
      ...
      for j in 0...2
        ...
        pbDrawEBDXOutlineText(@sprites["b#{i}"].bitmap,@btnEmp.width*j,11,@btnEmp.width,h,cmds[i],Color.white,c,1)
      end
