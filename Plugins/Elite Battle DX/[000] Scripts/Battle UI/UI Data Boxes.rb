...
class DataBoxEBDX  <  SpriteWrapper
  ...
  def refresh
    ...
    name_bitmap = @sprites["textName"].bitmap
    name_bitmap.clear
    pbSetSmallFont(name_bitmap)
    # Keep the three parts in stable columns. Long names shrink just enough to
    # fit instead of pushing the whole line beyond the data box.
    name_text   = @battler.name.nil? ? "" : @battler.name.to_s.force_encoding("UTF-8")
    gender_text = ""
    gender_text = _INTL("♂") if @pokemon.gender == 0 && !@hidden
    gender_text = _INTL("♀") if @pokemon.gender == 1 && !@hidden
    level_text  = "Lv.#{@battler.level}"
    text_left   = 18
    text_right  = name_bitmap.width - 22
    available_width = text_right - text_left
    loop do
      required_width = name_bitmap.text_size(name_text).width
      required_width += name_bitmap.text_size(gender_text).width + 4 if gender_text != ""
      required_width += name_bitmap.text_size(level_text).width + 10
      break if required_width <= available_width || name_bitmap.font.size <= 18
      name_bitmap.font.size -= 1
    end
    ...
    pbDrawEBDXOutlineText(name_bitmap, text_left, 7, available_width, name_bitmap.height,
                      name_text, color, Color.new(0,0,0), 0)
    # writes the Pokemon's gender
    gender_x = text_left + name_bitmap.text_size(name_text).width + 4
    color = (@pokemon.gender == 0) ? Color.new(53,107,208) : Color.new(180,37,77)
    pbDrawEBDXOutlineText(name_bitmap, gender_x, 7, available_width, name_bitmap.height,
                      gender_text, color, Color.new(0,0,0), 0)
    # writes the Pokemon's level
    pbDrawEBDXOutlineText(name_bitmap, text_left, 7, available_width, name_bitmap.height,
                      level_text, Color.white, Color.new(0,0,0), 2)
...
class SafariDataBoxEBDX < SpriteWrapper
  ...
  def refresh
    ...
    pbDrawEBDXOutlineText(self.bitmap,0,38,self.bitmap.width,self.bitmap.height,str,Color.white,Color.new(0,0,0,125),1)
  end
  #-----------------------------------------------------------------------------
  #  update (temp)
  #-----------------------------------------------------------------------------
