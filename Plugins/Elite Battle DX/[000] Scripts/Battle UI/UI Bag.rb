...
class BagWindowEBDX
  ...
  def checkPockets
    @mergedPockets = []
    if $bag.pockets.is_a?(Hash)
      $bag.pockets.each_value do |items|
        @mergedPockets.concat(items) if items
      end
    else
      $bag.pockets.each do |items|
        @mergedPockets.concat(items) if items
      end
    end
  end
  ...
  def drawPocket(pocket, index)
    ...
    pbDisposeSpriteHash(@items)
    # v22 stores Bag pocket metadata in GameData::BagPocket and no longer has
    # Settings.bag_pocket_names. EBDX displays its own four battle categories.
    @pname = [
      _INTL("Medicine"),
      _INTL("Poké Balls"),
      _INTL("Berries"),
    ...
    for i in 0...@pocket.length
      ...
      # draw texxt
      text = [
        ["#{GameData::Item.get(@pocket[i][0]).name}", pbmp.width/2 - 15, 2*pbmp.height/64, 2, @baseColor, @shadowColor],
        ["x#{@pocket[i][1]}", pbmp.width/2 - 12, 8*pbmp.height/64, 2, @baseColor, @shadowColor],
      ]
  ...
  def useItem?
    ...
    # draw text
    drawTextEx(bitmap, 80, 12, 364, 3, GameData::Item.get(@ret).description, @baseColor, @shadowColor)
  ...
  def refresh(skip = false)
    ...
    icon.dispose
    # draw text
    dtext = [[text[i], bmp.width/2, 14, 2, @baseColor, @shadowColor]]
  ...
  def confirm
    ...
    if @index < 4
      ...
      @page = -1
      @ret = @lastUsed
      @lastUsed = 0 if !($bag.quantity(@lastUsed) > 1)
    end
  end
  #-----------------------------------------------------------------------------
    ...
    loop do
      ...
      if !@bagWindow.ret.nil? && @bagWindow.useItem?
        ...
        when 1, 2, 3, 6, 7, 8   # Use on Pokémon/Pokémon's move/battler
          ...
          # v22 uses UI::Party instead of PokemonParty_Scene/Screen.
          @bagWindow.clearSel
          @bagWindow.hide
          idxParty = -1
          partyScreen = UI::Party.new(modParty, mode: :battle_use_item)
          partyScreen.choose_pokemon do |pkmn, partyIndex|
            idxParty = partyIndex
            next true if partyIndex < 0
            idxPartyRet = -1
            partyPos.each_with_index do |pos, i|
              next if pos != partyIndex + partyStart
              idxPartyRet = i
              break
            end
            next false if idxPartyRet < 0
            next false if !pkmn || pkmn.egg?
            idxMove = -1
            if useType == 2 || useType == 7   # Use on Pokémon's move
              idxMove = partyScreen.choose_move(pkmn, _INTL("Restore which move?"))
              next false if idxMove < 0
            end
            next true if yield item.id, useType, idxPartyRet, idxMove, partyScreen
            idxParty = -1
            next false
          end
          @bagWindow.show if idxParty < 0
          break if idxParty >= 0
