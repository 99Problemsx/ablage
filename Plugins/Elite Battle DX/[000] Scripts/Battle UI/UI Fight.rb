...
class FightWindowEBDX
  ...
  def initialize(viewport = nil, battle = nil, scene = nil)
    ...
    for i in 0...4
      pbDrawEBDXOutlineText(@typeInd.bitmap,0,24*i + 5,192,24,eff[i],Color.white,Color.black,1)
    end
  ...
  def generateButtons
    ...
    for i in 0...@nummoves
      ...
      pbDrawEBDXOutlineText(@button["#{i}"].bitmap, 198, 10, 196, 42,"#{movedata.name}", Color.white, baseColor, 1)
      pp = "#{@moves[i].pp}/#{@moves[i].total_pp}"
      pbDrawEBDXOutlineText(@button["#{i}"].bitmap, 0, 48, 191, 26, pp, Color.white, baseColor, 2)
      pbSetSystemFont(@button["#{i}"].bitmap)
      selectedMoveNameYPos = 18
      # v22's translated name lookup can return a string-compatible message
      # object. pbDrawTextPositions is stricter and requires an actual String.
      text = [[movedata.name.to_s, 99, selectedMoveNameYPos, 2, Color.white, Color.new(32, 32, 32)]]
